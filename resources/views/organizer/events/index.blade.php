...
<div style="margin-bottom: var(--space-8); display: flex; justify-content: space-between; align-items: flex-end;">
    <div>
        <h1 class="display" style="margin-bottom: var(--space-1); color: var(--slate-900);">Manajemen Event</h1>
        <p class="body" style="color: var(--slate-500);">Kelola event, tiket, dan status persetujuan.</p>
    </div>
...
<div class="card" style="padding: 0; overflow: hidden;">
                    ...
                    <tr>
                        <td style="border-left: none;">
                            <div style="display: flex; align-items: center; gap: var(--space-3);">
                                @if($event->image)
                                    <img src="{{ Storage::url($event->image) }}" style="width: 48px; height: 48px; border-radius: var(--radius-sm); object-fit: cover; border: 1px solid var(--slate-200);" alt="{{ $event->title }}">
                                @else
                                ...
                                @endif
                                <div>
                                    <div style="font-weight: 600; color: var(--slate-900); max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $event->title }}">{{ $event->title }}</div>
                        ...
                        <td>
                            <div style="font-weight: 500; color: var(--slate-800);">{{ \Carbon\Carbon::parse($event->start_date)->format('d M Y, H:i') }}</div>
    ...
    @if ($events->hasPages())
        <div style="padding: var(--space-4) var(--space-6); border-top: 1px solid var(--slate-100);">
            {{ $events->links() }}
        </div>
    @endif
</div>

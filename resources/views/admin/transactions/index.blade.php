...
<div style="max-width: 1200px; margin: 0 auto;">
    <div style="margin-bottom: var(--space-8);">
        ...
        <p class="body" style="color: var(--slate-400);">@yield('page_subtitle')</p>
    </div>
    ...
    <div class="card" style="padding: 0; overflow: hidden;">
        <div style="padding: var(--space-6); border-bottom: 1px solid var(--slate-700); background-color: var(--purple-500);">
            <div style="display: flex; align-items: center; gap: var(--space-4);">
                ...
                <div>
                    <h3 class="h3" style="margin: 0; color: #ffffff;">DAFTAR TRANSAKSI</h3>
                </div>
            </div>
        </div>
        ...
        <div style="overflow-x: auto;">
            <table class="table" style="margin: 0; border: none; box-shadow: none;">
                ...
                <tbody>
                    ...
                    <tr style="background-color: {{ $trx->status == 'pending' ? 'var(--slate-900)' : 'transparent' }};">
                        <td style="border-left: none; font-weight: 700; color: var(--slate-400); text-align: center;">{{ $index + 1 }}</td>
                        <td>
                            <span class="badge" style="font-family: monospace; font-size: 14px; background-color: {{ $trx->status == 'pending' ? 'var(--slate-700)' : 'var(--slate-900)' }}; color: {{ $trx->status == 'pending' ? 'var(--slate-200)' : 'var(--slate-0)' }}; border-color: {{ $trx->status == 'pending' ? 'var(--slate-600)' : 'var(--slate-600)' }};">
                                {{ $trx->order_id }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td>
                            @if($trx->status === 'settlement' || $trx->status === 'success')
                                <span class="badge" style="background-color: var(--feedback-success); color: var(--slate-0); border-color: #ffffff;">SUCCESS</span>
                            @elseif($trx->status === 'pending')
                                <span class="badge" style="background-color: var(--slate-700); color: var(--purple-500); border-color: var(--purple-700);">PENDING</span>
                            @else
                    ...
                    <tr>
                        <td colspan="7" style="text-align: center; padding: var(--space-10); border: none;">
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                                ...
                                <p class="body" style="color: var(--slate-400);">Belum ada data transaksi yang masuk.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div style="padding: var(--space-4) var(--space-6); border-top: var(--border-width-default) solid var(--slate-600); background-color: var(--slate-800); display: flex; justify-content: space-between; align-items: center;">
            <p class="caption" style="font-weight: 700; color: var(--slate-200); margin: 0;">TOTAL: {{ $transactions->total() }} TRANSAKSI</p>

...
<div style="max-width: 1000px; margin: 0 auto;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-8); flex-wrap: wrap; gap: var(--space-4);">
        <div>
            ...
            <p class="body" style="color: var(--slate-400);">Kelola semua acara, tiket, dan kapasitas dengan mudah.</p>
        </div>
    ...
    @if(session('success'))
        <div style="background-color: var(--feedback-success); color: var(--slate-0); padding: var(--space-4); border: 1px solid var(--slate-700); margin-bottom: var(--space-6); display: flex; align-items: center; gap: var(--space-3); box-shadow: var(--shadow-hard-sm);">
    ...
    <div class="card" style="padding: 0; overflow: hidden;">
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; align-items: center; gap: var(--space-4);">
                                <div style="width: 48px; height: 60px; border: 2px solid var(--slate-600); box-shadow: var(--shadow-hard-sm); background-color: var(--slate-800); overflow: hidden;">
                                    <img src="{{ ($event->poster_path && Storage::disk('public')->exists($event->poster_path))
                                         ? asset('storage/' . $event->poster_path)
                                         : 'https://placehold.co/16x20' }}" style="width: 100%; height: 100%; object-fit: cover;">
                                </div>
                                <div>
                                    <p class="body" style="font-weight: 700; color: var(--slate-0); margin: 0; text-transform: uppercase;">{{ $event->title }}</p>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td style="border-right: none; text-align: right;">
                            <div style="display: flex; justify-content: flex-end; gap: var(--space-2);">
                                <a href="{{ route('admin.events.edit', $event->id) }}" class="btn" style="padding: var(--space-2); background-color: var(--slate-700); color: var(--slate-0); border: 2px solid var(--slate-600);">
                        ...
                        <td colspan="5" style="text-align: center; padding: var(--space-10); border: none;">
                            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center;">
                                <div style="width: 80px; height: 80px; border: 4px solid var(--slate-600); background-color: #ffffff; display: flex; align-items: center; justify-content: center; margin-bottom: var(--space-4); box-shadow: var(--shadow-hard-sm);">
                                    <svg width="40" height="40" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square" viewBox="0 0 24 24" style="color: var(--slate-400);">
                                ...
                                <p class="body" style="color: var(--slate-400);">Mulai dengan menambahkan event pertama.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div style="padding: var(--space-4) var(--space-6); border-top: var(--border-width-default) solid var(--slate-600); background-color: var(--slate-800);">
            <p class="caption" style="font-weight: 700; color: var(--slate-200);">TOTAL: {{ $events->count() }} EVENT</p>
        </div>
    </div>
</div>
@endsection

...
@section('content')
    ...
    <section class="page-container d-flex align-center gap-4" style="padding-top: var(--space-10); padding-bottom: var(--space-10); flex-wrap: wrap;">
        <div style="flex: 1 1 500px; padding-right: var(--space-4);">
            ...
            <h1 class="display" style="margin-bottom: var(--space-4);">
                TEMUKAN & PESAN <br>
                <span style="color: var(--amber-500);">TIKET EVENT</span> IMPIANMU.
            </h1>
            <p class="body-lg" style="color: var(--ink-200); margin-bottom: var(--space-5); max-width: 500px;">
        ...
        <div style="flex: 1 1 400px; position: relative;">
            <div style="border: 4px solid var(--ink-700); border-radius: var(--radius-xl); box-shadow: var(--shadow-hard-modal); overflow: hidden; background-color: var(--ink-900);">
            ...
            <div class="alert alert-info" style="position: absolute; bottom: -20px; left: -20px; box-shadow: var(--shadow-hard-modal); border-color: var(--ink-700); background-color: var(--ink-950); padding: var(--space-2) var(--space-3); border-radius: var(--radius-md);">
                <div class="d-flex align-center gap-3">
                    <div style="width: 24px; height: 24px; background-color: var(--amber-500); border-radius: var(--radius-xs); display: flex; align-items: center; justify-content: center; color: var(--ink-950);">
                    ...
                    <div>
                        <p class="caption" style="color: var(--ink-400); margin:0;">TERVERIFIKASI</p>
                        <p class="label" style="color: var(--ink-0); margin:0;">Pembayaran via Midtrans</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <hr style="border: 0; border-top: var(--border-width-default) solid var(--ink-700); margin: var(--space-8) 0;">
    ...
    <section id="categories" class="page-container">
        <div style="text-center; margin-bottom: var(--space-8);">
            ...
            <p class="body" style="color: var(--ink-200); max-width: 600px; margin: 0 auto;">Temukan event yang sesuai minatmu dari berbagai kategori yang tersedia di AmikomEventHub.</p>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--space-4);">
            ...
            @endphp
            <a href="/?category={{ $cat->slug }}#events" class="card" style="text-decoration: none; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: var(--space-5) var(--space-3); color: var(--ink-0);">
                <div style="width: 48px; height: 48px; background-color: var(--amber-500); color: var(--ink-950); margin-bottom: var(--space-3); border-radius: var(--radius-xs); display: flex; align-items: center; justify-content: center; border: 2px solid var(--ink-950);">
                ...
                <div class="caption" style="color: var(--ink-400);">{{ $cat->events_count }} EVENT</div>
                @endif
            </a>
            @endforeach
        </div>
    ...
    <section id="events" class="page-container" style="padding-bottom: var(--space-10);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: var(--space-5);">
            ...
            <div class="card" style="padding: 0; display: flex; flex-direction: column; overflow: hidden;">
                <div style="position: relative; aspect-ratio: 16/9; border-bottom: 2px solid var(--ink-700);">
                    <img src="{{ ($event->poster_path && Storage::disk('public')->exists($event->poster_path))
                         ? asset('storage/' . $event->poster_path)
                         : 'https://placehold.co/600x400/141414/f5f5f0?text=EVENT+POSTER' }}" alt="{{ $event->title }}"
                         style="width: 100%; height: 100%; object-fit: cover; display: block;">
                    <div style="position: absolute; top: var(--space-2); left: var(--space-2); background: var(--amber-500); color: var(--ink-950); padding: 4px 8px; font-family: 'IBM Plex Mono', monospace; font-size: 11px; font-weight: 700; border: 2px solid var(--ink-950);">
                ...
                <div style="padding: var(--space-4); display: flex; flex-direction: column; flex: 1;">
                    ...
                    <div style="margin-bottom: var(--space-4); flex: 1;">
                        <div class="d-flex align-center gap-2 mb-2">
                            <svg width="16" height="16" fill="none" stroke="var(--ink-400)" stroke-width="2.25" stroke-linecap="square" viewBox="0 0 24 24">
                                <path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <span class="body-sm" style="color: var(--ink-200);">{{ \Carbon\Carbon::parse($event->date)->format('d M Y • H:i') }}</span>
                        </div>
                        <div class="d-flex align-center gap-2">
                            ...
                            <span class="body-sm" style="color: var(--ink-200);">TBA / Lihat Detail</span>
                        </div>
                    </div>

                    <div class="d-flex align-center justify-between" style="border-top: 1px solid var(--ink-700); padding-top: var(--space-4); margin-top: auto;">
                        <div>
                            <p class="caption" style="color: var(--ink-400); margin-bottom: 2px;">MULAI DARI</p>
                            <span class="h4" style="color: var(--amber-500); margin:0;">Rp {{ number_format($event->price, 0, ',', '.') }}</span>
                        </div>
        ...
        @if($events->isEmpty())
        <div style="padding: var(--space-8); border: 2px solid var(--ink-700); background-color: var(--ink-900); text-align: center; margin-top: var(--space-6);">
            <div style="width: 48px; height: 48px; border: 2px solid var(--ink-700); margin: 0 auto var(--space-4) auto; display: flex; align-items: center; justify-content: center;">
                <svg width="24" height="24" fill="none" stroke="var(--ink-400)" stroke-width="2.25" stroke-linecap="square" viewBox="0 0 24 24">
                    <path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                </svg>
            </div>
            <h3 class="h3" style="color: var(--ink-0); margin-bottom: var(--space-2);">BELUM ADA EVENT</h3>
            <p class="body" style="color: var(--ink-200);">Nantikan event seru berikutnya dari AmikomEventHub.</p>
        </div>
        @endif
    </section>
    ...
    <section id="partners" class="page-container" style="padding-bottom: var(--space-10);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: var(--space-4);">
            ...
            <div style="padding: var(--space-4); border: 2px solid var(--ink-700); display: flex; flex-direction: column; align-items: center; justify-content: center; background-color: var(--ink-900);">
                <div style="width: 64px; height: 64px; border: 2px solid var(--ink-700); margin-bottom: var(--space-3); display: flex; align-items: center; justify-content: center; overflow: hidden; background-color: var(--ink-950);">
                    <img src="{{ $partner->logo_url }}" alt="{{ $partner->name }}" style="max-width: 100%; max-height: 100%; filter: grayscale(100%) contrast(1.2);" onerror="this.parentElement.innerHTML='<span class=\'h4\' style=\'color: var(--ink-400);\'>{{ strtoupper(substr($partner->name, 0, 2)) }}</span>'">
                </div>
                <p class="label" style="text-align: center; color: var(--ink-0);">{{ $partner->name }}</p>
            </div>
            @endforeach
        </div>
    </section>
